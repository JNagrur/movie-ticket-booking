...
import { ProductComponent, ProductComponent2 } from "./ProductComponent";

// Static movie data — replaces the dead private API
const MOVIES = {
  latest: [
...
const ProductPage = () => {
  ...
  function loadMovies(type) {
    setActiveTab(type);
    dispatch(setProducts(MOVIES[type]));
  }

  useEffect(() => {
    dispatch(setProducts(MOVIES["latest"]));
  }, []);
  ...
  return (
    <>
      <div className="ui grid container">
        <div className="row">
          <div
            className={`col-4 nav-card ${activeTab === "latest" ? "nav-card-active" : ""}`}
            onClick={() => loadMovies("latest")}
          >
            Latest
          </div>
          <div
            className={`col-4 nav-card ${activeTab === "upcomingMovies" ? "nav-card-active" : ""}`}
            onClick={() => loadMovies("upcomingMovies")}
          >
            Upcoming
          </div>
          <div
            className={`col-4 nav-card ${activeTab === "events" ? "nav-card-active" : ""}`}
            onClick={() => loadMovies("events")}
          >
            Events
          </div>
        </div>
